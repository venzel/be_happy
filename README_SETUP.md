...
```bash
$ yarn add nodemon typescript tsconfig-paths @types/module-alias @types/node @types/express ts-node-dev npm-run-all eslint-import-resolver-typescript prettier eslint-config-prettier eslint-plugin-prettier @types/bcryptjs @types/jsonwebtoken @types/cors @types/helmet @types/morgan @types/nodemailer @types/bull @types/ioredis @types/multer jest @types/jest ts-jest -D
```

### Dependências de produção

```bash
$ yarn add express express-async-errors dotenv helmet morgan youch cowsay typeorm pg mongodb reflect-metadata bcryptjs date-fns jsonwebtoken cors @
```
